...
const newValid="function validTelugu(s,min,max){const c=normalize(s);const bad=[/\\b(?:సహనమును|ప్రయత్నమును|ఆత్మవిశ్వాసమును)\\s+(?:పెడతాయి|పెడుతుంది|పెడతాడు)/i,/\\b(?:బలాన్ని|ధైర్యాన్ని|నమ్మకాన్ని|సహనాన్ని)\\s+(?:పెడతాయి|పెడుతుంది|పెడతాడు)/i,/\\bమనకు\\s+బలాన్ని\\s+పెడ/i,/\\bవిజయానికి\\s+దారి\\s+తీస్తుంది/i];const blocked=bad.some(re=>re.test(c));return countWords(c)>=min&&countWords(c)<=max&&latin(c).length===0&&TELUGU_RANGE.test(c)&&/[.!?…]$/.test(c)&&!blocked}";

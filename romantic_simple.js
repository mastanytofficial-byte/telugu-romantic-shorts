...
function saveState(title) {
  ...
  fs.writeFileSync(STATE_FILE, JSON.stringify({ runCount: state.runCount + 1, usedTitles, lastDate: new Date().toISOString() }, null, 2));
}
...
function cleanScreen(text) {
  return String(text || '').replace(/^(SCREEN|QUOTE)\s*:\s*/i, '').replace(/#[^\s]+/g, '').replace(/["“”]/g, '').replace(/\s+/g, ' ').trim();
}
function cleanVoice(text) {
  return String(text || '').replace(/^(VOICE|SCRIPT)\s*:\s*/i, '').replace(/#[^\s]+/g, '').replace(/\s+/g, ' ').trim();
}
function parse(raw) {
...
async function generateQuote(runCount, usedTitles) {
  ...
  for (let attempt = 1; attempt <= 5; attempt++) {
    ...
    if (result.valid && parsed.imageQuery) return parsed;
  }
  throw new Error('Could not generate a valid Tenglish romantic quote after 5 attempts.');
}

function escapeSSML(text) { return text.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;'); }
...
async function synthesizeTelugu(text) {
  ...
  for (const sentence of (sentences.length ? sentences : [text])) {
    const res = await fetchWithTimeout(`https://texttospeech.googleapis.com/v1/text:synthesize?key=${GOOGLE_TTS_API_KEY}`, {
      ...
      body: JSON.stringify({ input: { ssml: `<speak><s>${escapeSSML(sentence)}</s></speak>` }, voice: { languageCode: 'te-IN', name: 'te-IN-Chirp3-HD-Achird' }, audioConfig: { audioEncoding: 'LINEAR16' } })
    });
    const data = await res.json();
  ...
  for (let i = 0; i < buffers.length; i++) {
    const p = path.join(WORK_DIR, `tts_${i}.wav`); fs.writeFileSync(p, buffers[i]); paths.push(p);
  }
  if (paths.length === 1) return paths[0];
  const list = path.join(WORK_DIR, 'tts_list.txt'); fs.writeFileSync(list, paths.map(p => `file '${p}'`).join('\n'));
  const out = path.join(WORK_DIR, 'voice.wav'); execSync(`ffmpeg -y -f concat -safe 0 -i "${list}" -c copy "${out}"`, { stdio: 'pipe' }); return out;
}

function writeWav(filePath, samples, sampleRate = 44100, channels = 2) {
  const dataSize = samples.length * 2; const buffer = Buffer.alloc(44 + dataSize);
  buffer.write('RIFF', 0); buffer.writeUInt32LE(36 + dataSize, 4); buffer.write('WAVE', 8); buffer.write('fmt ', 12); buffer.writeUInt32LE(16, 16); buffer.writeUInt16LE(1, 20);
  buffer.writeUInt16LE(channels, 22); buffer.writeUInt32LE(sampleRate, 24); buffer.writeUInt32LE(sampleRate * channels * 2, 28); buffer.writeUInt16LE(channels * 2, 32); buffer.writeUInt16LE(16, 34); buffer.write('data', 36); buffer.writeUInt32LE(dataSize, 40);
...
function generateOriginalBGM(duration, filePath) {
  const sr = 44100, total = Math.ceil(duration * sr), samples = new Int16Array(total * 2);
  const chords = [[261.63,329.63,392.00,493.88],[220.00,261.63,329.63,392.00],[174.61,220.00,261.63,329.63],[196.00,246.94,293.66,392.00]];
  const chordLength = 4.0;
  for (let i = 0; i < total; i++) {
    const t = i / sr, chord = chords[Math.floor(t / chordLength) % chords.length], local = (t % chordLength) / chordLength, fade = Math.min(1, t / 1.5, (duration - t) / 1.5);
    let value = 0;
    for (let n = 0; n < chord.length; n++) { const f = chord[n], slow = 0.55 + 0.45 * Math.sin(2 * Math.PI * (0.07 + n * 0.01) * t); value += Math.sin(2 * Math.PI * f * t) * 0.018 * slow; value += Math.sin(2 * Math.PI * (f * 2) * t) * 0.004 * slow; }
    value += Math.sin(2 * Math.PI * 0.5 * t) * 0.006 * (1 - local); value *= Math.max(0, Math.min(1, fade));
    const s = Math.round(value * 32767); samples[i * 2] = s; samples[i * 2 + 1] = s;
  }
...
async function fetchOneBackgroundImage(query) {
  ...
  try {
    const res = await fetchWithTimeout(pexelsUrl, { headers: { Authorization: PEXELS_API_KEY } }); const data = await res.json();
    if (!data.photos?.length) throw new Error('No Pexels photos returned');
    const photo = data.photos[Math.floor(Math.random() * data.photos.length)], imageUrl = photo.src?.large2x || photo.src?.large || photo.src?.original;
    const imageRes = await fetchWithTimeout(imageUrl, {}, 30000); if (!imageRes.ok) throw new Error(`Image download HTTP ${imageRes.status}`);
    const buf = Buffer.from(await imageRes.arrayBuffer()); if (buf.length < 5000) throw new Error('Downloaded image is too small');
    const out = path.join(WORK_DIR, 'background.jpg'); fs.writeFileSync(out, buf); log(`Using ONE Pexels background image: ${query}`); return out;
  } catch (e) {
    ...
    const res = await fetchWithTimeout(url, {}, 30000); if (!res.ok) throw new Error(`Pollinations image failed HTTP ${res.status}`);
    const buf = Buffer.from(await res.arrayBuffer()); if (buf.length < 5000) throw new Error('Fallback image is too small');
    const out = path.join(WORK_DIR, 'background.jpg'); fs.writeFileSync(out, buf); log('Using ONE Pollinations background image.'); return out;
  }
}
...
function buildVideo(imagePath, voicePath, bgmPath, screenText) {
  ...
  const duration = Number(execSync(`ffprobe -v error -show_entries format=duration -of csv=p=0 "${voicePath}"`).toString().trim()) + 0.4, fd = duration.toFixed(2);
  const vf = [
    `scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920`,
    `zoompan=z='min(zoom+0.0015,1.10)':d=${Math.ceil(duration * 25)}:s=720x1280:fps=25`,
    'eq=contrast=1.04:saturation=1.08','vignette=PI/6',
    'drawbox=x=0:y=0:w=iw:h=260:color=black@0.30:t=fill','drawbox=x=0:y=ih-300:w=iw:h=300:color=black@0.20:t=fill',
    ...
    `drawtext=fontfile='${fontPath}':text='TELUGU ECHO':fontcolor=white@0.45:fontsize=17:x=(w-text_w)/2:y=h-65`,
    'fade=t=in:st=0:d=0.5',`fade=t=out:st=${Math.max(0, duration - 0.5).toFixed(2)}:d=0.5`
  ].join(',');
  ...
  const cmd = ['ffmpeg -y',`-loop 1 -i "${imagePath}"`,`-i "${voicePath}"`,`-i "${bgmPath}"`,`-filter_complex "${filter}"`,`-map 0:v -map "[a]"`,`-vf "${vf}"`,`-t ${fd}`,'-c:v libx264 -pix_fmt yuv420p','-c:a aac -b:a 128k','-shortest',`"${out}"`].join(' ');
...
async function upload(videoPath, title, description) {
  const oauth2Client = new google.auth.OAuth2(YT_CLIENT_ID, YT_CLIENT_SECRET); oauth2Client.setCredentials({ refresh_token: YT_REFRESH_TOKEN });
  const youtube = google.youtube({ version: 'v3', auth: oauth2Client }); const safeTitle = sanitize(title || 'Telugu Romantic Quote', 95), safeDescription = sanitize(description, 4900);
  const res = await youtube.videos.insert({ part: ['snippet', 'status'], requestBody: { snippet: { title: safeTitle, description: safeDescription, tags: ['telugu quotes','telugu romantic quotes','tenglish quotes','telugu shorts','romantic shorts'], categoryId: '22' }, status: { privacyStatus: 'public', selfDeclaredMadeForKids: false } }, media: { body: fs.createReadStream(videoPath) } });
  log(`Uploaded: https://www.youtube.com/watch?v=${res.data.id}`);
}

async function main() {
  fs.mkdirSync(WORK_DIR, { recursive: true });
  for (const [name, value] of Object.entries({ GROQ_API_KEY, GOOGLE_TTS_API_KEY, PEXELS_API_KEY, YT_CLIENT_ID, YT_CLIENT_SECRET, YT_REFRESH_TOKEN })) if (!value) throw new Error(`${name} is missing`);
  const state = loadState(); log(`Run #${state.runCount + 1}: romantic quote, one image, one original BGM.`);
  const quote = await generateQuote(state.runCount, state.usedTitles); log(`Title: ${quote.title}`); log(`SCREEN (${countWords(quote.screen)} words): ${quote.screen}`); log(`VOICE: ${quote.voice}`); log(`IMAGE_QUERY: ${quote.imageQuery}`);
  const imagePath = await fetchOneBackgroundImage(quote.imageQuery), voicePath = await synthesizeTelugu(quote.voice);
  ...
  saveState(quote.title); log('Done.');
}
